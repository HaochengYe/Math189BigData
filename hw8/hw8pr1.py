...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for i in range(m):
			data = X[i, :]
			diff = data - clusters
			index = np.argsort(np.linalg.norm(diff, axis = 1)).item(0)
			label[i] = index
		for cluster in range(k):
			index = np.where(label == cluster)[0]
			if len(index) > 0:
				clusters[cluster, :] = X[index].mean(axis = 0)
...
if __name__ == '__main__':
	...
	optimal_cost, = plt.plot(opt_k, min(cost_k_list), 'rD')
